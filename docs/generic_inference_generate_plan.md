...
- Storyboard still owns its own scheduler/dispatcher in `tool/server/storyboard_generation.py`.
- Test Generations still owns a separate in-memory queue in `tool/server/epoch_test_bench.py` using `_pending_tests`, `_advance_test_queue()`, and its own worker tracking.
...
During this phase only Generate will be executed through the runner. Storyboard remains on its existing path until its migration phase. This allows the scheduler contract to be proven without destabilizing Storyboard.
...
- no Test migration;
- no Storyboard execution migration;
- no event bus;
