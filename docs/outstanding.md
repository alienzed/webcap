...
- H3 calibration now persists conclusive per-shape evidence in Settings, reuses it on fresh probe launches for matching total RAM/GPU model/VRAM hardware, derives direct known-safe 17f/34f/68f ceilings without mixed validation, and is owned by Training Settings rather than the media context menu. Generated H3 Normal/Quality defaults retain a two-rung margin below the exact selectable calibrated ceiling.
