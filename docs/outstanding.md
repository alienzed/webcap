...
- Review and Filter only seem to work on the initially loaded set of captions. I've found that after editing a caption and, for example, adding a unique word, that word will not show up in the Rare tokens until I refresh the directory. It feels like we're caching things but not updating the cache appropriately.
- After a rename, the item gets deselected. Ideally the new name could be reselect after refresh... when I'm tired I may genuinely forget which file I was working on, especially if they all have similar names
