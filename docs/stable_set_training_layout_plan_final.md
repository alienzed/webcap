...
- treat `actionId` as a POSIX relative identifier: parse it with `PurePosixPath` or equivalent platform-independent POSIX component logic before mapping validated components onto host `Path` objects;
- do not let platform-native path parsing define the persisted ID format;
...
It may live anywhere readable by the configured training environment **and resolvable through WebCap's existing training-path/WSL path helper**. Do not add a new remote-path or filesystem-access abstraction for Custom Resume.

### 9.2 Validation happens before training-artifact or queue mutation
...
The backend validates it as an action-contained POSIX relative path before use. Parse/validate it with `PurePosixPath` or equivalent platform-independent POSIX component logic before joining it to a host filesystem path.
...
- **stop deriving action identity from `action_root.name`**: nested version-2 actions require the full manifest/root-relative `actionId`;
- update concrete basename assumptions such as `_new_job()` assigning `"actionId": action_root.name`, managed Resume calling `read_action(resume_action.name)`, and capture recording calling `update_action(action_root.name, ...)`;
- carry the full action ID from the action manifest or an actions-root-relative helper through job/history payloads and `read_action()` / `update_action()` calls; the logical-run basename may still be used only for sequence/display parsing;
