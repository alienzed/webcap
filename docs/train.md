...
- Generated configs default to `<filesystem.root>/output/runs/<set-name>/<timestamp>/`. The active stage config's `output_dir` remains authoritative for run discovery and resume.
