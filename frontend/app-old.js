...
class MediaWeb {
    ...
    setupEventListeners() {
        ...
        // Text columns now added via placeholder inputs in the canvas
            ...
            deleteBtn.addEventListener('click', (e) => {
                ...
                if (confirm(`Delete page "${page.title}"?`)) {
                    ...
                    if (idx >= 0) {
                        ...
                        if (this.api) {
                            this.api.deletePage(page).catch(err => {
                                this.log('error', `Failed to delete page: ${err.message || err}`);
                            });
                        }
                        this.renderPagesList();
                    }
    ...
    setupCanvasDropZone(canvas) {
        ...
        canvas.addEventListener('drop', (e) => {
            ...
            this.log('info', `Drop event: mediaId=${mediaId}, mediaType=${mediaType}`);
    ...
    renderPageColumns() {
        ...
        // Add placeholder input at the end
        this.addColumnPlaceholder(canvas);
    }
    
    addColumnPlaceholder(canvas) {
        ...
        input.addEventListener('blur', () => {
            if (input.value.trim()) {
                ...
                page.columns.push(column);
                this.log('info', 'Added text column');
                this.renderPageColumns();
            } else {
    ...
    renderEditorMediaLibrary(filterText = '') {
        ...
        filtered.forEach(media => {
            ...
            item.style.cssText = 'cursor: grab; padding: 4px; border: 1px solid #ddd; border-radius: 4px; background: white; user-select: none;';
    ...
    editTextColumnInline(columnDiv, column) {
    ...
    async savePage() {
        ...
        this.log('info', `Saving page "${page.title}" (${page.columns.length} columns)`);
        ...
        if (this.api) {
            try {
                ...
                this.log('info', '✓ Page saved to disk');
            } catch (err) {
                this.log('error', `Failed to save page to disk: ${err.message || err}`);
            }
        }
    ...
    previewPage() {
        ...
        page.columns.forEach(column => {
            ...
            if (column.type === 'text') {
                colDiv.innerHTML = `<div style="padding: 15px; background: #f9f9f9; border-radius: 4px; white-space: pre-wrap; word-wrap: break-word;">${this.escapeHtml(column.data.content)}</div>`;
            } else if (column.type === 'image') {
                const media = this.media.find(m => m.id === column.data.media_id);
                if (media) {
                    const fileUrl = this.getMediaFileUrl(media);
