...
class MediaWeb {
    constructor() {
        ...
        this.debug = window.MEDIAWEB_DEBUG || false; // Global debug mode
        // Do not initialize modules here
    }

    async init() {
        if (this.debug) console.log('[MediaWeb] Debug mode enabled');
    ...
    setupEventListeners() {
        ...
        document.querySelectorAll('.nav-item').forEach(item => {
            item.addEventListener('click', (e) => {
                if (this.debug) console.log(`[MediaWeb] Nav click: ${item.getAttribute('data-section')}`);
    ...
    switchSection(e) {
        ...
        if (this.debug) console.log(`[MediaWeb] Switching to section: ${section}`);
        ...
        // Load section data
        if (section === 'media') {
            if (this.debug) console.log('[MediaWeb] Rendering media grid');
            this.mediaManager.renderGrid();
        } else if (section === 'pages') {
            if (this.debug) console.log('[MediaWeb] Rendering pages list');
            this.pageEditor.renderPagesList();
        } else if (section === 'settings') {
            if (this.debug) console.log('[MediaWeb] Updating settings display');
    ...
    async loadData() {
        ...
        try {
            ...
            // Load per-media metadata into an in-memory cache for sync access
            try {
                const metaPromises = this.media.map(m => this.api.getMediaMetadata(m.id, m.date).catch(() => null));
                const metas = await Promise.all(metaPromises);
                metas.forEach((meta, idx) => {
                    const id = this.media[idx].id;
                    this.mediaMetadata[id] = meta || {
                        title: '',
                        caption: '',
                        tags: [],
                        date: this.media[idx].date || null,
                        created: this.media[idx].created || new Date().toISOString(),
                        modified: this.media[idx].modified || new Date().toISOString(),
                        crop: null,
                        rotation: null
                    };
                });
            } catch (e) {
                this.console.log('warn', `Failed to pre-load some metadata: ${e.message || e}`);
            }
    ...
    getMediaMetadata(mediaId) {
        // Return metadata from in-memory cache populated from disk (fast/sync)
        const meta = this.mediaMetadata[mediaId];
        if (meta) return meta;
        return {
            title: '',
            caption: '',
    ...
    setMediaMetadata(mediaId, metadata) {
        // Update in-memory cache. Persisting to disk is handled by FileIOAPI where callers have access.
        this.mediaMetadata[mediaId] = metadata;
    }
...
// Global debug flag (set window.MEDIAWEB_DEBUG = true in devtools to enable extra logging)
window.MEDIAWEB_DEBUG = window.MEDIAWEB_DEBUG || false;
try {
    window.app = new MediaWeb();
    window.app.console = new Console(window.app);
    window.app.mediaManager = new MediaManager(window.app);
    window.app.pageEditor = new PageEditor(window.app);
    window.app.console.init();
    window.app.mediaManager.init();
    window.app.pageEditor.init();
    window.app.init();
    if (window.app.debug) console.log('[MediaWeb] App initialized');
} catch (err) {
