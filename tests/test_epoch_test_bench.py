...
def test_run_batch_marks_session_failed_when_template_load_fails(tmp_path, monkeypatch):
    session = tmp_path / "session"
    session.mkdir()
    bench._atomic_write_json(
        session / "test.json",
...
def test_epoch_test_bench_does_not_import_training_runner_at_module_load():
    ...
    top_level = source.split("def remove_candidate", 1)[0]
...
def test_queued_candidate_snapshot_rejects_changed_unmanaged_file(tmp_path):
    candidate = tmp_path / "epoch01.safetensors"
    ...
    with pytest.raises(RuntimeError, match="changed after it was queued"):
        bench._validate_queued_candidate_snapshots([candidate], snapshots)
...
def test_remove_candidate_refuses_when_queued_test_references_it(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="Portrait check"):
        bench.remove_candidate(tmp_path, candidate.name)

    assert candidate.is_file()


def test_direct_test_start_operation_is_removed():
    source = Path(bench.__file__).read_text(encoding="utf-8")
    assert 'operation == "test_start"' not in source
    assert "\ndef start(" not in source
