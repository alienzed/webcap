...
def test_removed_scene_discards_its_takes_but_keeps_scene_setup(storyboard_fs):
    story = storyboard_store.create_story({"title": "Story"})
    story, scene = storyboard_store.add_scene(
        story["id"],
        {"title": "Scene", "prompt": "Keep this setup.", "summary": "Summary."},
    )
    story, take = storyboard_store.add_take_upload(
        story["id"], scene["id"], "take.mp4", BytesIO(b"video")
    )
    ...
    removed = storyboard_store.delete_scene(story["id"], scene["id"])

    removed_scene = removed["removedScenes"][scene["id"]]
    assert removed_scene["title"] == "Scene"
    assert removed_scene["prompt"] == "Keep this setup."
    assert removed_scene["takes"] == {}
    assert removed_scene["removedTakes"] == {}
    assert removed_scene["takeOrder"] == []
    assert removed_scene["selectedTakeId"] is None
    assert not media.exists()

    restored = storyboard_store.restore_scene(story["id"], scene["id"])
    assert restored["scenes"][scene["id"]]["prompt"] == "Keep this setup."
...
def test_scene_removal_refuses_to_break_take_reference_used_by_another_scene(storyboard_fs):
    ...
    story, target = storyboard_store.add_scene(story["id"], {"title": "Target"})
    story, take = storyboard_store.add_take_upload(
        story["id"], source["id"], "source.png", BytesIO(b"image")
    )
    story, _reference = storyboard_store.set_scene_reference_from_take(
        story["id"], target["id"], "first_frame", source["id"], take["id"], "first"
    )

    with pytest.raises(RuntimeError, match="used as another Scene reference"):
        storyboard_store.delete_scene(story["id"], source["id"])

    saved = storyboard_store.load_story(story["id"])
    assert source["id"] in saved["scenes"]
    assert storyboard_store.resolve_story_media(story["id"], take["mediaPath"]).is_file()
