...
const savedMarker = svg.match(/class="training-candidates-epoch-marker training-candidates-saved-marker[^>]*>.*?<circle cx="([^"]+)" cy="([^"]+)" r="5"/);
...
assert(Math.abs(Number(savedMarker[1]) - Number(firstEpochLossPoint[0])) < .01);
assert(Math.abs(Number(savedMarker[2]) - Number(firstEpochLossPoint[1])) < .01);
