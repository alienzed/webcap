...
function wireTrainingCandidatesChart() {
  ...
  chart.addEventListener('mousemove', function (event) {
    ...
    var y = Math.max(20, Math.min(270, 20 + (data.maxLoss - Number(raw.loss)) / (data.maxLoss - data.minLoss) * 250));
    ...
    tooltip.style.top = Math.max(4, Math.min(rect.height - 150, event.clientY - rect.top + 10)) + 'px';
