...
function renderTrainingHistory() {
  ...
  var jobs = (history.jobs || []).filter(function (job) {
    ...
    var haystack = [
      job.folder, job.datasetTarget, job.profileId, job.stages, job.status, job.modelLabel, model.label, model.source
    ].join(' ').toLowerCase();
  ...
  els.historyList.innerHTML = visibleJobs.map(function (job) {
    ...
    var canResume = job.status !== 'cancelled' && job.outputAvailable !== false && !!resumePath && ['hi', 'lo', 'krea2', 'wan21'].indexOf(resumeStage) !== -1;
    var unavailable = [];
...
function resumeTrainingHistoryJob(jobId) {
  ...
  if (!job || !job.folder || !resumePath || ['hi', 'lo', 'krea2', 'wan21'].indexOf(resumeStage) === -1) {
