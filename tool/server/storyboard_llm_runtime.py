...
PRELOAD_VRAM_HEADROOM_MIB = 2048
...
def _inference_has_launchable_work():
    from .inference_runner import snapshot as inference_snapshot

    current = inference_snapshot(include_terminal=False)
...
def _gpu_free_mib():
    ...
    if not snapshot.get("available") or not isinstance(gpus, list) or not gpus:
        return None
    primary = gpus[0] if isinstance(gpus[0], dict) else {}
    try:
        total = float(primary.get("memoryTotal"))
        used = float(primary.get("memoryUsed"))
...
def _preload_required_mib(model):
    path = Path(str(model.get("path") or "")).expanduser()
    try:
        size_mib = path.stat().st_size / (1024 ** 2)
    except OSError:
        return None
    return size_mib * PRELOAD_MODEL_OVERHEAD_RATIO + PRELOAD_VRAM_HEADROOM_MIB


def preload_model(model_id):
    settings = _director_config()
    if settings.get("mode", "local") == "remote":
        return {"loaded": False, "reason": "remote", "model": str(model_id or "")}

    with _request_lock:
        _ensure_server()
        model = _model_record(model_id)
        ...
        try:
            ...
            if free_mib is None:
                return {"loaded": False, "reason": "gpu_memory_unknown", "model": model["id"]}
            if required_mib is None:
                return {"loaded": False, "reason": "model_size_unknown", "model": model["id"]}
            if free_mib < required_mib:
                return {
                    ...
                    "model": model["id"],
                    "freeMiB": free_mib,
                    "requiredMiB": required_mib,
                }

            _set_activity("preparing", model_id=model["id"], operation="preload", active=True, error="")
            try:
                loaded = _ensure_local_model_loaded(model["id"])
                _set_activity("complete", model_id=model["id"], operation="preload", active=False)
                return {
                    "loaded": bool(loaded),
                    "reason": "loaded" if loaded else "already_loaded",
                    "model": model["id"],
                    "freeMiB": free_mib,
                    "requiredMiB": required_mib,
                }
            except Exception as exc:
                _set_activity("error", model_id=model["id"], operation="preload", active=False, error=str(exc))
                try:
                    if _model_status(model["id"]) != "unloaded":
                        _unload_model(model["id"])
