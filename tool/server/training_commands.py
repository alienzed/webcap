...
def build_training_command_plan(hi_config_path, lo_config_path, launcher=DEEPSPEED_LAUNCHER, resume_from_checkpoint="", resume_stage=""):
    ...
    hi_command = (
        ...
        + (resume_option if resume_stage == "hi" else "")
    )
    lo_command = (
        'NCCL_P2P_DISABLE="1" NCCL_IB_DISABLE="1" '
        + resolved_launcher
        ...
        + (resume_option if resume_stage == "lo" else "")
    )
    return {
